...
// Structural stand-in for the Claude message payload found in history
// JSONL lines. Previously typed via the agent SDK's SDKUserMessage /
// SDKAssistantMessage; the gateway no longer depends on the SDK, and
// this loose shape covers everything the parsers actually access
// (role, id, content).
export interface RawHistoryMessage {
  role?: "user" | "assistant" | string;
  /** Only assistant messages carry an id. */
  id?: string;
  content?: unknown;
  [key: string]: unknown;
}
  ...
  try {
    ...
    for (const line of lines) {
      try {
        ...
        // Only assistant messages (BetaMessage) carry `id`; user MessageParam
        // doesn't, so we narrow via `in` before accessing.
        if (
