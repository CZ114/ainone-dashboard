...
function loadEnvFiles(): void {
  ...
  const candidates = [
    ...
    path.join(repoRoot, "backend", "agent_gateway", ".env"),
  ];
...
async function main(runtime: NodeRuntime) {
  ...
  // No Claude CLI validation anymore — AI work is proxied to the
  // Python agent service (AGENT_SERVICE_URL, default 127.0.0.1:8100).
  // cliPath is a legacy config field kept only for wire compatibility.
