...
/** Proxy an upstream JSON response through unchanged. */
...
/**
 * GET /api/sessions — proxied to the Python agent service, which owns
 * session storage now. Returns { sessions: SessionSummary[] } already
 * in frontend shape.
 */
export async function handleSessionList(_c: Context) {
  return proxyJson(`${agentServiceUrl()}/api/compat/sessions`);
}
...
/**
 * GET /api/sessions/:sessionId/messages — proxied to the Python agent
 * service. Returns { messages: [...], cwd } already in frontend shape.
 */
export async function handleSessionMessages(c: Context) {
  ...
  return proxyJson(
    `${agentServiceUrl()}/api/compat/sessions/${encodeURIComponent(sessionId)}/messages`,
  );
}
