...
export function restoreTimestamps(
  ...
  // Only assistant messages (BetaMessage) carry `id`; the `in` guard lets
  // TypeScript narrow the MessageParam|BetaMessage union to the variant
  // that actually has the property.
  const hasId = (
    msg: RawHistoryLine,
  ): msg is RawHistoryLine & { message: { id: string } } =>
    msg.type === "assistant" &&
    !!msg.message &&
    "id" in msg.message &&
