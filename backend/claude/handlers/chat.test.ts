...
describe("Chat Handler - Permission Mode Tests", () => {
  ...
  describe("Permission Mode Parameter Handling", () => {
    it("should pass permissionMode 'plan' to Claude SDK", async () => {
      ...
      // handleChatRequest returns the Response synchronously; the stream's
      // start() runs the generator asynchronously, so we have to wait for
      // query() to actually be reached before asserting its args.
    ...
    it("should handle permissionMode alongside other parameters", async () => {
      // Use process.cwd() — a real directory — because the handler now
      // sanity-checks the cwd exists (via statSync) before spawning.
      // A made-up "/project/path" would short-circuit to an error
      // before query() ever runs.
  ...
  describe("Message Processing with Permission Mode", () => {
    it("should process slash commands with permissionMode", async () => {
      ...
      // The backend MUST pass the leading `/` through so the CLI
      // recognizes it as a built-in slash command (/help, /compact,
      // etc.). An earlier version stripped the slash, which disabled
      // every real CLI slash command.
      expect(mockQuery).toHaveBeenCalledWith({
        prompt: "/help",
        options: expect.objectContaining({
          permissionMode: "plan",
  ...
  describe("Abort Controller Management with Permission Mode", () => {
    ...
    it("should store and retrieve abort controller during execution", async () => {
      ...
      await handleChatRequest(mockContext, requestAbortControllers);
      await vi.waitFor(() => expect(capturedController).toBeInstanceOf(AbortController));
