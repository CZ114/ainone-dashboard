...
export function parseCliArgs(): ParsedArgs {
  ...
  // Get default port from environment (default 3000 to avoid conflict with Python backend on 8080)
  const defaultPort = parseInt(getEnv("PORT") || "3000", 10);

  // Configure program
  program
    .name("ainone-claude-backend")
    .version(version, "-v, --version", "display version number")
    .description("AinOne Dashboard — Claude Code backend server")
    .option(
      "-p, --port <port>",
      "Port to listen on",
      (value) => {
