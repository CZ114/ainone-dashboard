 ...
 * string fails (spawn doesn't auto-append .exe). Probes standard install
 * locations and returns the first hit.
 *
 * Unicode safety: paths are built from `os.homedir()` which Node gets
 * via the Win32 W-API and preserves non-ASCII chars correctly.
 */
...
async function* executeClaudeCommand(
  ...
  try {
    ...
    // Load user env from settings.json
    ...
    //
    // Build query options:
    //   - `systemPrompt: preset 'claude_code'` loads CLAUDE.md files.
    //   - `settingSources: ['project', 'user', 'local']` reads settings.json.
    //   - `tools: preset 'claude_code'` enables the default tool set.
    //
    // CLI path: the SDK spawns a native binary without shell, so an absolute
    // path is required on Windows (no .exe auto-append, no PATH lookup).
